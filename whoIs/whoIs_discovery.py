...
def _extract_emails(whois_record):
    ...
    # Fallback: zoek e-mails in ruwe WHOIS-tekst als structured field leeg is
    if not emails:
        text_blob = ""
...
def find_related_domains_by_term(term):
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"[LOOKUP WARNING] ViewDNS gaf status {res.status_code} voor '{term}'")
            return []
...
def discover_more_domains(domain):
    ...
    # 1 — Op basis van email
    if whois_data["emails"]:
        for email in whois_data["emails"]:
            lookup_terms.append(email)
            more = find_related_domains_by_term(email)
            for d in more:
                found_domains.add(d)
    else:
        print("[WHOIS INFO] Geen WHOIS e-mail gevonden; probeer fallback op naam/org.")

    # 2 — Fallback op WHOIS naam en organisatie
    for term in _as_list(whois_data.get("name")) + _as_list(whois_data.get("org")):
        if len(term) >= 4:
            lookup_terms.append(term)
            for d in find_related_domains_by_term(term):
                found_domains.add(d)

    if not lookup_terms:
        print("[WHOIS INFO] Geen bruikbare WHOIS-termen (email/naam/org) beschikbaar voor reverse lookup.")

    if not found_domains:
        print("[WHOIS INFO] Geen gerelateerde domeinen gevonden via openbare reverse-WHOIS bronnen.")

    # Je kan later ook org-based discovery toevoegen via andere API's
    found_domains.discard(domain.lower())
    return sorted(found_domains)


if __name__ == "__main__":
    domain = sys.argv[1] if len(sys.argv) > 1 else "foederer.nl"
    print("[WHOIS] Scanning:", domain)

    related = discover_more_domains(domain)
    print("\n[FOUND DOMAINS]")
    for d in related:
        print(" -", d)